...
const uint64_t BMagics[64] = {
    0x0006EFF5367FF600ULL,
    0x00345835BA77FF2BULL,
    0x00145F68A3F5DAB6ULL,
    0x003A1863FB56F21DULL,
    0x0012EB6BFE9D93CDULL,
    0x000D82827F3420D6ULL,
    0x00074BCD9C7FEC97ULL,
    0x000034FE99F9FFFFULL,
    0x0000746F8D6717F6ULL,
    0x00003ACB32E1A3F7ULL,
    0x0000185DAF1FFB8AULL,
    0x00003A1867F17067ULL,
    0x0000038EE0CCF92EULL,
    0x000002A2B7FF926EULL,
    0x000006C9AA93FF14ULL,
    0x00000399B5E5BF87ULL,
    0x00400F342C951FFCULL,
    0x0020230579ED8FF0ULL,
    0x007B008A0077DBFDULL,
    0x001D00010C13FD46ULL,
    0x00040022031C1FFBULL,
    0x000FA00FD1CBFF79ULL,
    0x000400A4BC9AFFDFULL,
    0x000200085E9CFFDAULL,
    0x002A14560A3DBFBDULL,
    0x000A0A157B9EAFD1ULL,
    0x00060600FD002FFAULL,
    0x004006000C009010ULL,
    0x001A002042008040ULL,
    0x001A00600FD1FFC0ULL,
    0x000D0ACE50BF3F8DULL,
    0x000183A48434EFD1ULL,
    0x001FBD7670982A0DULL,
    0x000FE24301D81A0FULL,
    0x0007FBF82F040041ULL,
    0x000040C800008200ULL,
    0x007FE17018086006ULL,
    0x003B7DDF0FFE1EFFULL,
    0x001F92F861DF4A0AULL,
    0x000FD713AD98A289ULL,
    0x000FD6AA751E400CULL,
    0x0007F2A63AE9600CULL,
    0x0003FF7DFE0E3F00ULL,
    0x000003FD2704CE04ULL,
    0x00007FC421601D40ULL,
    0x007FFF5F70900120ULL,
    0x003FA66283556403ULL,
    0x001FE31969AEC201ULL,
    0x0007FDFC18AC14BBULL,
    0x0003FB96FB568A47ULL,
    0x000003F72EA4954DULL,
    0x00000003F8DC0383ULL,
    0x0000007F3A814490ULL,
    0x00007DC5C9CF62A6ULL,
    0x007F23D3342897ACULL,
    0x003FEE36EEE1565CULL,
    0x0003FF3E99FCCCC7ULL,
    0x000003ECFCFAC5FEULL,
    0x00000003F97F7453ULL,
    0x0000000003F8DC03ULL,
    0x000000007EFA8146ULL,
    0x0000007ED3E2EF60ULL,
    0x00007F47243ADCD6ULL,
...
void init_piece(bool rook, int sq)
{
    ...
    do
    {
        ...
        assert((Attacks[sq][index] == 0) || (Attacks[sq][index] == calc_attacks(sq, occ)));
...
void init_bitboard()
{
    ...
    for (sq = 0; sq < 64; ++sq)
    {
        ...
        PawnAttacks[0][sq] = calc_pawn_attacks(SquareMask[sq], 0);
        PawnAttacks[1][sq] = calc_pawn_attacks(SquareMask[sq], 1);
    }
}
/*******************************************************
Nom ......... : msb
...
{
    uint64_t const h1 = ((knights >> 1) & 0x7F7F7F7F7F7F7F7FULL) | ((knights << 1) & 0xFEFEFEFEFEFEFEFEULL);
    uint64_t const h2 = ((knights >> 2) & 0x3F3F3F3F3F3F3F3FULL) | ((knights << 2) & 0xFCFCFCFCFCFCFCFCULL);
...
{
    occ &= DiagMaskEx[sq];
    return ((occ - SquareMask[sq]) ^ byteswap(byteswap(occ) - SquareMask[sq ^ 56])) & DiagMaskEx[sq];
}
/*******************************************************
Nom ......... : adiag_attacks
Role ........ :
...
{
    occ &= ADiagMaskEx[sq];
    return ((occ - SquareMask[sq]) ^ byteswap(byteswap(occ) - SquareMask[sq ^ 56])) & ADiagMaskEx[sq];
}
/*******************************************************
Nom ......... : calc_bishop_attacks
Role ........ :
Arg ......... : sq      :  
                occ      :
Return....... : uint64_t
